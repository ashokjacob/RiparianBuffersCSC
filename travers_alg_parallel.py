...
wd = r"D:\Ashok\Catskills_Project\Inputs"
...
fdr_file = os.path.join(wd,"FDR10m.tif")  # flow direction "D:\Ashok\Catskills_Project\Inputs\FDR10m.tif"
lc_file = os.path.join(wd,"LU10m.tif")  # land cover, ccap, original classification "D:\Ashok\Catskills_Project\Inputs\LU10m.tif"
dist_mask_file = os.path.join(wd, 'Mask200m.tif') #"D:\Ashok\Catskills_Project\Inputs\Mask200m.tif"
output_prefix = 'CNVL_Out'

removalrate_forest = 0.9 #.6#0.9 #0.58 #0.9
removalrate_nonforest = 0.7 #0.55#0.7 #0.20 #0.7
...
for o,v in {
    ...
    outfl = os.path.join('outputs','{}_{}.tif'.format(output_prefix, o))
    print(outfl)
    outDs = driver.Create(
        outfl,
        lc.shape[1],
